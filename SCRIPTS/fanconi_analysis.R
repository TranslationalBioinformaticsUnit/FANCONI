...
celltype<-c("HSC","LMPP","Cycling_LMPP","GMP1","Monocytes","DC","CLP","MEP","Erythroid","Basophils")

#>>Load DEA results for cell_type and do the  GSEA on each on of them


for (i in 1:length(celltype)){
  set.seed(1234567)
  dea_res<-read.table(paste0("Fanconi_eltrombopag/Corrected_vs_uncorrected_",celltype[i],".txt"))
...
#  pdf(paste0("Fanconi_sample/DEA_all//ridgeplot_GO_",celltype[i],".pdf"), width = 15)
#  
#  ridgeplot(gse)
#  dev.off()
  
  write.table(gse,paste0("Fanconi_eltrombopag/DEA_all/GO_",celltype[i],".txt", sep=""),
...
for (i in 1:length(celltype)){
  ...
  HS_Genes<-merge(dea_res,anno2, by.x = 0, by.y="hgnc_symbol")
  ranked_genes<-HS_Genes[,paste0('avg_logFC_',celltype[i])]
  names(ranked_genes)<-HS_Genes$entrezgene_id
  ranked_genes <- sort(ranked_genes, decreasing = T)
  head(ranked_genes)
  tail(ranked_genes)
  
  KEGG<-gseKEGG(ranked_genes, organism = "human", keyType = "kegg", nPerm = 1000, minGSSize = 1, maxGSSize = 500, pvalueCutoff = 1)
  
  write.table(KEGG,paste0("Fanconi_eltrombopag/DEA_all/KEGG_",celltype[i],".txt", sep=""),
  sep="\t",quote=F,row.names=F)
  
  #>> HALLMARK pathways
  gmtfile_H <- read.gmt('h.all.v7.1.entrez.gmt')
  
  egmt <- GSEA(ranked_genes, TERM2GENE=gmtfile_H, verbose=F, minGSSize = 1, maxGSSize = 500, pvalueCutoff = 1)
  
  write.table(egmt,paste0("Fanconi_eltrombopag/DEA_all/HALLMARK_",celltype[i],".txt", sep=""),
