...
# STEP 1.1: LOAD THE DATA Of different contrast
Celltype<-c("LMPP","Cycling_LMPP","GMP1","GMP2","Monocytes","DC","CLP","PreB","MEP","Erythroid","Basophils")

for (i in 1:length(Celltype)){

C1<-read.table(paste0("/datos_2/FANCONI/Fanconi_sample/corrected_vs_not_corrected_", Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C2_1<-read.table(paste0("/datos_2/FANCONI/Integration_updated/MO_193/GSEA_corrected/corrected_vs_healthy_", Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C2_2<-read.table(paste0("/datos_2/FANCONI/Integration_updated/MO_194/GSEA_corrected/corrected_vs_healthy_", Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C2_3<-read.table(paste0("/datos_2/FANCONI/Integration_updated/MO_196/GSEA_corrected/corrected_vs_healthy_", Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C2_all<-read.table(paste0("/datos_2/FANCONI/Integration_updated/GSEA_all_corrected/corrected_vs_healthy_",Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C3_1<-read.table(paste0("/datos_2/FANCONI/Integration_updated/MO_193/GSEA_not_corrected/not_corrected_vs_healthy_",Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C3_2<-read.table(paste0("/datos_2/FANCONI/Integration_updated/MO_194/GSEA_not_corrected/not_corrected_vs_healthy_",Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C3_3<-read.table(paste0("/datos_2/FANCONI/Integration_updated/MO_196/GSEA_not_corrected/not_corrected_vs_healthy_",Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C3_all<-read.table(paste0("/datos_2/FANCONI/Integration_updated/GSEA_all_not_corrected/not_corrected_vs_healthy_",Celltype[i],".txt",sep=""),sep="\t", header=TRUE)
C4<-read.table(paste0("/datos_2/FANCONI/Public_data/Fanconi_vs_healthy_",Celltype[i],".txt",sep=""),sep="\t", header=TRUE)

# TAKE ROWNAMES OF EACH CONTRAST
C1<-rownames(C1)
C2_1<-rownames(C2_1)
C2_2<-rownames(C2_2)
C2_3<-rownames(C2_3)
C2_all<-rownames(C2_all)
C3_1<-rownames(C3_1)
C3_2<-rownames(C3_2)
C3_3<-rownames(C3_3)
C3_all<-rownames(C3_all)
C4<-rownames(C4)

background <- unique(c(C1, C2_1, C2_2, C2_3,C2_all,C3_1,C3_2,C3_3,C3_all,C4))
...
dea$C1 <- ifelse(rownames(dea) %in% C1, 1, 0)
dea$C2_1 <- ifelse(rownames(dea) %in% C2_1, 1, 0)
dea$C2_2 <- ifelse(rownames(dea) %in% C2_2, 1, 0)
dea$C2_3 <- ifelse(rownames(dea) %in% C2_3, 1, 0)
dea$C2_all <- ifelse(rownames(dea) %in% C2_all, 1, 0)
dea$C3_1 <- ifelse(rownames(dea) %in% C3_1, 1, 0)
dea$C3_2<- ifelse(rownames(dea) %in% C3_2, 1, 0)
dea$C3_3 <- ifelse(rownames(dea) %in% C3_3, 1, 0)
dea$C3_all <- ifelse(rownames(dea) %in% C3_all, 1, 0)
dea$C4 <- ifelse(rownames(dea) %in% C4, 1, 0)
...
ha1 <- HeatmapAnnotation(
  Contrast = colnames(dea),
    col = list( Contrast = c("C1"=my_palette1[2],"C2_1"=my_palette2[1],
                       "C2_2"=my_palette2[2],"C2_3"=my_palette2[3],"C2_all"=my_palette2[4],
                       "C3_1"=my_palette2[6],"C3_2"=my_palette2[7],
                       "C3_3"=my_palette2[8], "C3_all"=my_palette2[9], "C4"=my_palette1[1])),
  show_annotation_name = TRUE
)


pdf("/datos_2/FANCONI/TABLES/LMPP_heatmap.pdf")
Heatmap(dea, name = "Diff_genes", top_annotation = ha1, 
...
library(xlsx)
write.xlsx(dea, paste0("/datos_2/FANCONI/TABLES/Diff_genes_",Celltype[i],".xlsx",sep=""), sheetName = "Sheet1", col.names = TRUE, row.names = TRUE, append = FALSE)
}

##Fanconi patient

dea_LMPP_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_LMPP.txt", sep="\t", header=T)
dea_Cycling_LMPP_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_Cycling_LMPP.txt", sep="\t", header=T)
dea_GMP1_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_GMP1.txt", sep="\t", header=T)
dea_GMP2_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_GMP2.txt", sep="\t", header=T)
dea_Monocytes_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_Monocytes.txt", sep="\t", header=T)
dea_DC_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_DC.txt", sep="\t", header=T)
dea_CLP_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_CLP.txt", sep="\t", header=T)
dea_PreB_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_PreB.txt", sep="\t", header=T)
dea_MEP_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_MEP.txt", sep="\t", header=T)
dea_Erythroid_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_Erythroid.txt", sep="\t", header=T)
dea_Basophils_fanconi<-read.table("TABLES/Fanconi_0.2_Celltype/markers_fanconi_positive_vs_negative_Basophils.txt", sep="\t", header=T)
